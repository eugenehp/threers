...
use threers::{
    ...
    parse_scad_file, parse_scad_file_for_parts, parts_to_glb,
};

fn main() {
    let args: Vec<String> = std::env::args().skip(1).collect();
    let float = args.iter().any(|a| a == "--float");
    let parts = args.iter().any(|a| a == "--parts");
    let pos: Vec<&String> = args.iter().filter(|a| !a.starts_with("--")).collect();
    if pos.len() < 2 {
        eprintln!("usage: scad2stl [--float] [--parts] <in.scad> <out.{{stl,obj,off,3mf,glb}}>");
        std::process::exit(2);
    }
    let (input, output) = (pos[0], pos[1]);
    let ext = output.rsplit('.').next().unwrap_or("").to_ascii_lowercase();

    let solid = if parts && ext == "glb" {
        match parse_scad_file_for_parts(input) {
            Ok(s) => s,
            Err(e) => {
    ...
    let (bytes, tris, kernel) = if parts && ext == "glb" {
        let pieces = if float {
            solid.parts_float()
        } else {
            solid.parts()
        };
        let tris: usize = pieces
            .iter()
            .map(|p| {
                p.geometry
                    .index
            ...
            .sum();
        let bytes = parts_to_glb(&pieces);
        (
            bytes,
            tris,
            if float { "float+parts" } else { "exact+parts" },
        )
    } else {
        let geom = if float {
            solid.to_geometry()
        ...
        (bytes, tris, if float { "float" } else { "exact" })
    };

    if ext == "glb" && parts && bytes.is_empty() {
        eprintln!("error: --parts produced no geometry");
        std::process::exit(2);
    }

    std::fs::write(output, &bytes).expect("write output");
    println!(
        "parsed {input} → {output} · {tris} triangles · {} bytes · {kernel} kernel",
        bytes.len(),
    );
}
