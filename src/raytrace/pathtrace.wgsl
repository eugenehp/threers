...
struct Uniforms {
    ...
    limits: vec4<u32>,
    // seed low, seed high, hemisphere count, flags (bit0 environment, bit1 opaque shadows)
    flags: vec4<u32>,
...
// One map slot out of a material: 8 floats — rect, then the UV offset/repeat.
fn map_sample(base: u32, slot: u32, uv: vec2<f32>, wrap: u32) -> vec4<f32> {
    let o = base + 24u + slot * 8u;
...
fn perturb_normal(tri: u32, base: u32, ns: vec3<f32>, uv: vec2<f32>) -> vec3<f32> {
    ...
    let scale = vec2<f32>(data[base + 64u], data[base + 65u]);
...
fn bsdf_make(s: Surface, n: vec3<f32>, geom: vec3<f32>) -> Bsdf {
    ...
    let total = max(w_diffuse + w_specular + w_transmission + w_coat, 1e-6);
...
fn eval_transmission(b: Bsdf, wo: vec3<f32>, wi: vec3<f32>) -> Eval {
    ...
    if (reflecting) {
        ...
        out.f = vec3<f32>(v, v, v) * weight;
        out.pdf = vndf_reflect_pdf(wo, wm, b.ax, b.ay) * pr / max(pr + pt, 1e-9);
        return out;
    }
    ...
    // Radiance scales with the square of the relative index across an
    // interface; camera-side transport divides it back out.
    let v = d * (1.0 - fr) * g * abs(dot(wi, wm) * dot(wo, wm) / (cos_i * cos_o * denom)) / (etap * etap);
    let dwm_dwi = abs(dot(wi, wm)) / denom;
    out.f = b.s.base_color * (v * weight);
    out.pdf = vndf_pdf(wo, wm, b.ax, b.ay) * dwm_dwi * pt / max(pr + pt, 1e-9);
    return out;
}
...
fn eval_local(b: Bsdf, wo: vec3<f32>, wi: vec3<f32>) -> Eval {
    ...
    if (b.p_diffuse > 0.0 && reflecting) {
        ...
        out.f = out.f + b.s.base_color * ((1.0 - b.s.metallic) * (1.0 - b.s.transmission) * kd * under_coat / PI);
    ...
    if (!spec_smooth && reflecting) {
        ...
        let g = smith_g2(wo, wi, b.ax, b.ay);
        let fr = schlick(b.f0, abs(dot(wo, h)));
    ...
    if (pick < acc) {
        ...
        if (pick < acc) {
            if (spec_smooth) {
                ...
                is_delta = true;
                // f*cos/pdf must come out as the Fresnel term, so with pdf = 1
                // the value carries the 1/cos the caller will cancel.
                delta_value = schlick(b.f0, abs(wo.z)) * (b.spec_weight / max(abs(wi.z), 1e-6));
            } else {
            ...
            if (pick < acc) {
                ...
                // Split reflection from refraction in proportion to Fresnel and
                // divide it back out: unbiased either way, and a grazing ray
                // reflects nearly always, as it should.
                let do_reflect = rand(rng) < fr;
                if (do_reflect) {
                    wi = reflect_about(-wo, h);
                } else {
                    var n = h;
                ...
                if (spec_smooth) {
                    ...
                    if (do_reflect) {
                        let v = fr * weight / max(abs(wi.z), 1e-6);
                        delta_value = vec3<f32>(v, v, v) / max(fr, 1e-6);
                    } else {
                        let etap = select(1.0 / eta, eta, wo.z > 0.0);
                        let v = (1.0 - fr) * weight / (etap * etap * max(abs(wi.z), 1e-6));
                        delta_value = b.s.base_color * v / max(1.0 - fr, 1e-6);
                    }
                }
            } else if (b.p_coat <= 0.0) {
                // The three probabilities above sum to 1 when there is no coat,
                // but only to within rounding — a `pick` a hair under 1 can
                // still land here, and shading a lobe of zero weight kills the
                // path for nothing.
                return out;
            } else {
                if (coat_smooth) {
...
fn visibility(o: vec3<f32>, d: vec3<f32>, t_max_in: f32, eps: f32) -> vec3<f32> {
    ...
    for (var layer = 0u; layer <= max_layers; layer = layer + 1u) {
        ...
        let base = uni.data_offsets.z + mat * MAT_STRIDE;
        // Refractive glass blocks: light that really gets through it arrives
        // along BSDF-sampled paths, not along a straight line.
...
fn camera_ray(px: vec2<f32>, lens: vec2<f32>) -> Ray {
    let u = px.x / f32(uni.dims.x);
    let v = px.y / f32(uni.dims.y);
    // WebGPU clip space runs z from 0 at the near plane to 1 at the far one.
    let ndc = vec2<f32>(u * 2.0 - 1.0, 1.0 - v * 2.0);
    let near = unproject(vec3<f32>(ndc, 0.0));
    let far = unproject(vec3<f32>(ndc, 1.0));

    var r: Ray;
    if (uni.cam_pos.w > 0.5) {
        r.o = uni.cam_pos.xyz;
        let dd = far - r.o;
        r.d = select(uni.cam_forward.xyz, normalize(dd), dot(dd, dd) > 1e-20);
    } else {
        r.o = near;
        let dd = far - near;
        r.d = select(uni.cam_forward.xyz, normalize(dd), dot(dd, dd) > 1e-20);
    ...
    let aperture = uni.cam_right.w;
    let focus = uni.cam_up.w;
    if (aperture > 0.0 && focus > 0.0) {
        let c = dot(r.d, uni.cam_forward.xyz);
        if (abs(c) > 1e-6) {
            let focal_point = r.o + r.d * (focus / c);
            let l = concentric_disk(lens);
            r.o = r.o + uni.cam_right.xyz * (l.x * aperture) + uni.cam_up.xyz * (l.y * aperture);
...
fn trace_path(ray_in: Ray, rng: ptr<function, Sampler>) -> PathResult {
    ...
    for (var step = 0u; step < max_steps; step = step + 1u) {
        ...
        if (!bsdf_is_delta(b)) {
            // --- one analytic light, scaled by the count. Direct lighting then
            // costs the same whether the scene has one light or a hundred.
                ...
                if (ls.valid) {
                    ...
                    if (cos > 0.0 && dot(e.f, e.f) > 0.0) {
                        ...
                        if (dot(vis, vis) > 0.0) {
                            ...
                            res.radiance = res.radiance + clamp_contribution(c, bounce);
                        }
                    }
                }
            }
            ...
            if (!world_is_black()) {
                ...
                if (wpdf > 1e-9) {
                    let radiance = world_lighting(wdir);
                    if (dot(radiance, radiance) > 0.0) {
                        ...
                        if (cos > 0.0 && dot(e.f, e.f) > 0.0) {
                            ...
                            if (dot(vis, vis) > 0.0) {
                                ...
                                let c = throughput * e.f * radiance * vis * (cos * mis / wpdf);
                                res.radiance = res.radiance + clamp_contribution(c, bounce);
                            }
                        }
                    }
...
fn adaptive_converged(color: vec3<f32>, lum_sq: f32, n: f32) -> bool {
    ...
    return sqrt(variance / n) / max(mean, 1e-3) < threshold;
}
...
fn trace(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let base = pixel * ACCUM_STRIDE;

    // Read the running totals in, add to them, write them back once. The
    // convergence test needs the totals *including* this dispatch's samples, so
    // they have to be live in registers rather than read-modify-written per
    // channel at the end.
    var color = vec3<f32>(accum[base], accum[base + 1u], accum[base + 2u]);
    var alpha = accum[base + 3u];
    var albedo = vec3<f32>(accum[base + 4u], accum[base + 5u], accum[base + 6u]);
    var normal = vec3<f32>(accum[base + 7u], accum[base + 8u], accum[base + 9u]);
    var depth = accum[base + 10u];
    var depth_n = accum[base + 11u];
    var taken = accum[base + 12u];
    var lum_sq = accum[base + 13u];

    let first = uni.dims.z;
    let count = uni.dims.w;
    for (var s = 0u; s < count; s = s + 1u) {
        if (adaptive_converged(color, lum_sq, taken)) {
            break;
        }
        // Seed from (pixel, sample index, render seed) alone, so the result does
        // not depend on how the work was batched or scheduled.
        var smp: Sampler;
        smp.rng = hash32((gid.x * 1973u) ^ (gid.y * 9277u) ^ ((first + s) * 26699u) ^ uni.flags.x);
        smp.seed = hash32((gid.x * 0x9e3779b9u) ^ (gid.y * 0x85ebca6bu) ^ uni.flags.x);
        smp.index = first + s;
        // The camera owns dimension block 0; shading at bounce `b` takes block
        // `b + 1`.
        sampler_set_bounce(&smp, 0u);
        let jitter = rand2(&smp);
        let lens = rand2(&smp);
        let r = camera_ray(vec2<f32>(f32(gid.x), f32(gid.y)) + jitter, lens);
        let path = trace_path(r, &smp);
        color = color + path.radiance;
        alpha = alpha + path.alpha;
        albedo = albedo + path.albedo;
        normal = normal + path.normal;
        let lum = dot(path.radiance, vec3<f32>(0.2126, 0.7152, 0.0722));
        lum_sq = lum_sq + lum * lum;
        taken = taken + 1.0;
        if (path.depth < INF) {
            depth = depth + path.depth;
            depth_n = depth_n + 1.0;
        }
    }

    accum[base] = color.r;
    accum[base + 1u] = color.g;
    accum[base + 2u] = color.b;
    accum[base + 3u] = alpha;
    accum[base + 4u] = albedo.r;
    accum[base + 5u] = albedo.g;
    accum[base + 6u] = albedo.b;
    accum[base + 7u] = normal.x;
    accum[base + 8u] = normal.y;
    accum[base + 9u] = normal.z;
    accum[base + 10u] = depth;
    accum[base + 11u] = depth_n;
    accum[base + 12u] = taken;
    accum[base + 13u] = lum_sq;
}
